...
class HeartyGlyphLightSettingsPage {
    ...
    public function create_admin_page() {
     ...
     ?>
        ...
        <div class="hearty-admin">
            <h1 class="hearty-admin-title">Hearty Glyph Light | Settings</h1>
            <h5 class="hearty-admin-description"><a href="https://www.heartyplugins.com/wordpress-plugins/free-plugins/product/hearty-glyph-light" target="_blank">See details and documentation</a></h5>
            <form method="post" action="options.php" class="hearty-admin-form" autocomplete="off">
                <?php
                    settings_fields('heartyglyphlight_options_group');
                    do_settings_sections('heartyglyphlight-setting-admin');
                    submit_button();
                ?>
            </form>
        </div>
    ...
    public function page_init() {
        ...
        $this->options = get_option('heartyglyphlight_options');
        $this->number_of_settings_instances = 1;
        $this->number_of_content_items = 4;

        register_setting(
            'heartyglyphlight_options_group',
            'heartyglyphlight_options',
            array($this, 'sanitize')
        );

        $this->generate_sections();

        HeartyGlyphLightParams::generate_fields($this, $this->number_of_settings_instances, $this->number_of_content_items);
	...
	public function generate_sections() {
		...
		for ($i = 1; $i <= $this->number_of_content_items; $i++) {
			...
			add_settings_section(
				'heartyglyphlight_content_options_section_'.$i,
    ...
    public function fields_callback($args) {
        ...
        if ($args['type'] == "text") {
        	...
        	$output .= '<input type="text" class="'.$args["class"].'" id="'.$args["id"].'" name="heartyglyphlight_options['.$args["id"].']" value="'.(isset($this->options[$args["id"]]) ? esc_attr($this->options[$args["id"]]) : $args["default"]).'" />';

        } else if ($args['type'] == "textarea") {

        	$output .= '<textarea id="'.$args["id"].'" name="heartyglyphlight_options['.$args["id"].']">'.(isset($this->options[$args["id"]]) ? esc_attr($this->options[$args["id"]]) : $args["default"]).'</textarea>';

        } else if ($args['type'] == "select") {

        	$output .= '<select id="'.$args["id"].'" name="heartyglyphlight_options['.$args["id"].']">';
            if (!isset($args["force"])) {
        	...
        	$svalue = isset($this->options[$args["id"]]) ? esc_attr($this->options[$args["id"]]) : $args["default"];
        	$output .= '<input type="hidden" id="'.$args["id"].'" name="heartyglyphlight_options['.$args["id"].']" value="'.$svalue.'" />';
    ...
    public function media_callback($args) {
        ...
        if ($args['type'] == "text") {
			...
			$output .= '<input id="'.$args["id"].'" type="text" name="heartyglyphlight_options['.$args["id"].']" value="'.(isset($this->options[$args["id"]]) ? esc_attr($this->options[$args["id"]]) : $args["default"]).'" />';
